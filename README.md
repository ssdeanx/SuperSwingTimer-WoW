...
| Shaman | Lightning Bolt / Chain Lightning / Healing Wave / Lesser Healing Wave / Chain Heal weave markers with per-family toggles, plus a moving active-cast spell icon pinned to the projected MH landing point |
...
- Shaman weave assist for Lightning Bolt, Chain Lightning, Healing Wave, Lesser Healing Wave, and Chain Heal breakpoints, with color-coded family timing, small breakpoint spell icons that stay above and below the MH bar texture, an active cast icon that moves to the projected landing point on the current MH swing so you can see whether the spell will land before MH, spell-name resolution through the shared spell-info wrapper, and clean hide/reset behavior when Minimal Mode, the weave toggle, or the cast state disables it
...
| Shaman weave | Small breakpoint markers stay above the MH bar fill, and an active tracked-spell icon pins to the projected impact point on the current swing | Makes both the safe start point and the real cast landing point readable without covering the swing bar |
...
| Enhancement Shaman | MH + OH | Weave assist for LB, CL, HW, LHW, and CH breakpoints with spell-haste fallback support (`UnitSpellHaste` → `GetSpellHaste`), a moving active-cast icon pinned to projected impact timing on the MH bar, Windfury ICD tracking, and a Flurry stack badge |
