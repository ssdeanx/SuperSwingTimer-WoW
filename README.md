...
- Rogue Slice and Dice helper: a slim duration bar above the MH bar that uses the shared bar texture path, matches MH width, tracks the active Slice and Dice buff in real time from `UnitAura`, and stays hidden when the buff is down
- Rogue test energy-tick helper: a 5px vertical bar to the left of the MH/OH stack that fills upward on the Classic/TBC 2-second energy cadence, matches the visible melee-bar heights (23px at the stock 15px MH + 8px OH profile), re-syncs itself when likely natural energy gains are observed, and has a dedicated `Rogue Energy Helper` checkbox in the top Quick Controls
...
| Rogue | MH + OH + SnD helper | Dual-wield tracking plus a latency-adjusted MH end-window cue for Sinister Strike timing, a slim Slice and Dice duration bar above MH, and a test vertical energy tick helper |
