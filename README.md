...
| Shaman | Lightning Bolt / Chain Lightning / Healing Wave / Lesser Healing Wave / Chain Heal weave markers with per-family toggles, plus a moving active-cast spell icon that animates from the live cast-start point and resizes with current haste/buffs toward the projected MH landing point while the safe breakpoint markers stay fixed, a 6px Flame Shock target-duration bar above MH (checkbox-toggleable), and a Lightning Shield / Water Shield 3-charge tracker with improved aura-refresh wiring (checkbox-toggleable) |
...
| Shaman weave | Small breakpoint markers stay fixed above the MH bar fill, and the active tracked-spell icon/spark animates from the live cast-start point while rescaling with current haste toward the projected impact point on the current swing | Keeps the safe weave breakpoint readable while making the real cast position update immediately instead of looking pinned to one static helper point |
...
| Enhancement Shaman | MH + OH | Weave assist for LB, CL, HW, LHW, and CH breakpoints with spell-haste fallback support (`UnitSpellHaste` → `GetSpellHaste`), fixed breakpoint markers plus a moving active-cast icon/spark that starts from the live cast-start point and continuously resizes with current haste/buffs toward projected impact timing on the MH bar, Windfury ICD tracking, a Flurry stack badge, a Flame Shock target-duration bar above MH (6px stock height), and a Lightning Shield / Water Shield 3-bar charge tracker to the left of the MH bar with configurable color |
