...
| Shaman | Lightning Bolt / Chain Lightning / Healing Wave / Lesser Healing Wave / Chain Heal weave markers with per-family toggles, plus a moving active-cast spell icon that travels from the safe breakpoint toward the projected MH landing point |
...
- Shaman weave assist for Lightning Bolt, Chain Lightning, Healing Wave, Lesser Healing Wave, and Chain Heal breakpoints, with color-coded family timing, small breakpoint spell icons that stay above and below the MH bar texture, an active cast icon and center spark that start at the safe breakpoint and travel toward the projected landing point on the current MH swing so you can see whether the spell stays ahead of MH throughout the cast, spell-name resolution through the shared spell-info wrapper, and clean hide/reset behavior when Minimal Mode, the weave toggle, or the cast state disables it
...
| Shaman weave | Small breakpoint markers stay above the MH bar fill, and the active tracked-spell icon/spark moves from the safe start point toward the projected impact point on the current swing | Makes both the safe start point and the real cast landing point readable without covering the swing bar, while also showing live cast progress against the MH swing |
...
| Enhancement Shaman | MH + OH | Weave assist for LB, CL, HW, LHW, and CH breakpoints with spell-haste fallback support (`UnitSpellHaste` → `GetSpellHaste`), a moving active-cast icon/spark that travels from the breakpoint toward projected impact timing on the MH bar, Windfury ICD tracking, and a Flurry stack badge |
