...
- Shaman weave help can keep the upper/lower breakpoint markers visible at the safe start point while also moving the active tracked spell icon from the real cast-start position toward its projected landing point on the current main-hand swing, with the motion continuously rescaled by current spell haste/buffs, so players can see both where to start the cast and where Lightning Bolt or Chain Lightning currently sits against MH as the cast progresses
- Enhancement Shaman support can also expose a thin Flame Shock duration bar directly above MH, tracking your own Flame Shock on the current target so shock upkeep is visible without replacing the main swing timer
...
v1.2.5 - Update this number when making changes to the code, and update the CHANGELOG.md with details of the changes.
