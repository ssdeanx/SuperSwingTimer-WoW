...
- Shaman weave help can keep the upper/lower breakpoint markers visible while also moving the active tracked spell icon from the safe breakpoint toward its projected landing point on the current main-hand swing, so players can see both where to start the cast and whether Lightning Bolt or Chain Lightning stays ahead of MH as the cast progresses
