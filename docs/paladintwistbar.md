# [LvL] Paladin Twistbar

```lua
-- Do not remove this comment, it is part of this aura: [LvL] GCD tick
local swingRegion = WeakAuras.GetRegion("[LvL]  MainHand Swing Bar (Twist)")
local swingBar = swingRegion and swingRegion.bar

aura_env.swingBar = swingBar
aura_env.swingSpeed = UnitAttackSpeed("player") or 2.0
aura_env.swingExpiration = 0

aura_env.UpdateGCDTick = function()
    local bar = aura_env.swingBar
    ...
    local start, duration = GetSpellCooldown(29515)
    if not start or duration == 0 then
        if aura_env.gcdTick then aura_env.gcdTick:Hide() end
        return
    end
    local gcd_remaining = (start + duration) - now
    if gcd_remaining <= 0 then
        if aura_env.gcdTick then aura_env.gcdTick:Hide() end
        return
    end
    
    if aura_env.swingExpiration == 0 then
        if aura_env.gcdTick then aura_env.gcdTick:Hide() end
        return
    end
    ...
    local swingRemaining = aura_env.swingExpiration - now
    local swingElapsed
    if swingRemaining <= 0 then
        swingElapsed = 0
    else
        swingElapsed = speed - swingRemaining
        if swingElapsed < 0 then swingElapsed = 0 end
    end
    
    local gcd_end = (swingElapsed + gcd_remaining) / speed
    if gcd_end > 1 then
        gcd_end = gcd_end - 1
    end
    
    if not aura_env.gcdTick then
        aura_env.gcdTick = CreateFrame("Frame", nil, bar)
        aura_env.gcdTick:SetFrameLevel(bar:GetFrameLevel() - 1)
        aura_env.gcdTick.bg = aura_env.gcdTick:CreateTexture(nil, "ARTWORK")
        aura_env.gcdTick.bg:SetAllPoints(aura_env.gcdTick)
    end
    
    aura_env.gcdTick:SetFrameLevel(bar:GetFrameLevel() - 1)
    aura_env.gcdTick.bg:SetDrawLayer("ARTWORK")
    aura_env.gcdTick.bg:SetColorTexture(0.6, 0.2, 1, 1)
    aura_env.gcdTick:SetHeight(h)
    aura_env.gcdTick:SetWidth(2)
    aura_env.gcdTick:ClearAllPoints()
    aura_env.gcdTick:SetPoint("LEFT", bar, "LEFT", w * gcd_end, 0)
    aura_env.gcdTick:Show()
end

aura_env.HideGCDTick = function()
    if aura_env.gcdTick then aura_env.gcdTick:Hide() end
end

-- Do not remove this comment, it is part of this aura: [LvL]  MainHand Swing Bar (Twist)
...
aura_env.elapsedTime = 0
aura_env.startTime = 0
aura_env.elapsedTime = GetTime() - aura_env.startTime



-- Do not remove this comment, it is part of this aura: [LvL] SwingTimerAPI
...
local reset_swing_spells = {
    ...
    [461607] = true, -- Divine Steed
    [31935] = true, -- Avenger's Shield
    [32699] = true, -- Avenger's Shield
    [32700] = true, -- Avenger's Shield
    [24275] = true, -- Hammer of Wrath
    [24274] = true, -- Hammer of Wrath
    [24239] = true, -- Hammer of Wrath
    [27180] = true, -- Hammer of Wrath
}
...
function lib:COMBAT_LOG_EVENT_UNFILTERED(_, ts, subEvent, _, sourceGUID, _, _, _, destGUID, _, _, _, amount, overkill, _, resisted, _, _, _, _, _, isOffHand)
    ...
    if subEvent == "SPELL_EXTRA_ATTACKS" and sourceGUID == self.unitGUID then
        -- // This is causing corrupted mainSpeed value when interacting with deprecated parry haste code
        self.mainSpeed = ts
        --
        -- // This stapled-on "extra attack fix" is now redundant and causing problems with stored attacks
        self.skipNextAttackCount = resisted
        self.mainExpirationTime = now
    ...
    elseif (subEvent == "SWING_DAMAGE" or subEvent == "SWING_MISSED") and sourceGUID == self.unitGUID then
        ...
        -- // Seemingly more dead code for TBC extra attacks
        if
        self.skipNextAttack ~= nil
    ...
    elseif subEvent == "SWING_MISSED" and amount ~= nil and amount == "PARRY" and destGUID == self.unitGUID then
        ...
        local now = GetTime()
        local remaining = self.mainExpirationTime - now
        local reduction = 0.4 * self.mainSpeed
        local minRemaining = 0.2 * self.mainSpeed
        local newRemaining = (remaining - reduction) < minRemaining and minRemaining or (remaining - reduction)
        self.mainExpirationTime = now + newRemaining
        self:Fire("SWING_TIMER_UPDATE", self.mainSpeed, self.mainExpirationTime, "mainhand")
        if self.mainSpeed > 0 and (self.mainExpirationTime - now) > 0 then
