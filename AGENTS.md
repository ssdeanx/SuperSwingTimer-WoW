...
- v0.0.7 rogue helper follow-up (2026-05-17): the Rogue energy helper is now 6px wide for slightly better readability, and the Slice and Dice bar now reads player helpful auras through a Classic-safe `UnitBuff` / `UnitAura` signature-tolerant helper so the SnD bar shows correctly on current Classic/TBC clients.
