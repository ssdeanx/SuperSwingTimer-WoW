...
- **Current focus**: v0.0.9 final pass — every buff-duration and helper bar across all 6 classes now has matching `/sst` controls (show/hide toggle + height/width slider). Added Druid Power Shift Height + Energy Tick Width + Rogue Adrenaline Rush Height sliders; added Druid Power Shift + Energy Tick visibility toggles. All 8 helper bars (Warrior Shield Block, Hunter Rapid Fire, Hunter Range Helper, Rogue SnD, Rogue Energy Tick, Rogue Adrenaline Rush, Druid Power Shift, Druid Energy Tick) now wired in `/sst`.
