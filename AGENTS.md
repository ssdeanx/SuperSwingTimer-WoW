...
## Current progress (2026-05-31 alias sweep)

- v0.1.2 class-init fail-open hardening (2026-06-01): extended class setup protection in `ns.InitClassMods()` so all class initializers (`Paladin`, `Warrior`, `Rogue`, `Hunter`, `Shaman`, `Druid`) execute through a safe wrapper. This prevents a class helper init error from suppressing core bar visibility for that class. Shaman retains targeted fallback cleanup for helper-only visuals if setup fails.

- v0.1.2 final class audit (2026-05-31): completed a file-by-file review of Warrior, Hunter, Rogue, Shaman, and Druid runtime paths in `SuperSwingTimer_ClassMods.lua` plus the supporting state/weave/UI hooks. No new crash-level blocker was found in the live class blocks; the remaining work for test is in-game smoke validation of class-specific helper overlays and combat-state transitions. No version bump; addon remains pinned to `v0.1.2` for final user testing.

- v0.1.2 timing clock cleanup (2026-05-31): reviewed the `GetTimePreciseSec` / `GetTime` clock path across the addon and normalized the last inline fallback smell in live runtime code. `SuperSwingTimer.lua` and `SuperSwingTimer_Config.lua` now use shared local clock helpers instead of repeating `GetTimePreciseSec() or GetTime()` inline, and the matching hidden worktree UI copy was aligned too. No version bump; addon remains pinned to `v0.1.2` for final test.

- v0.1.2 broader alias sweep across non-ClassMods Lua files (2026-05-31): audited `SuperSwingTimer.lua`, `SuperSwingTimer_State.lua`, `SuperSwingTimer_Weaving.lua`, `SuperSwingTimer_UI.lua`, and `SuperSwingTimer_Config.lua` for lowercased Blizzard API alias smells and redundant rawget wrapper patterns. The only remaining non-ClassMods alias smell found was `GetSpellCooldownLegacy` in `SuperSwingTimer_State.lua`; it has been removed and the code now uses the existing top-level `GetSpellCooldown` alias directly. No version bump; still pinned to `v0.1.2` for final test.
...
- v0.1.2 broader alias sweep across non-ClassMods Lua files (2026-05-31): audited `SuperSwingTimer.lua`, `SuperSwingTimer_State.lua`, `SuperSwingTimer_Weaving.lua`, `SuperSwingTimer_UI.lua`, and `SuperSwingTimer_Config.lua` for lowercased Blizzard API alias smells and redundant rawget wrapper patterns. Most files were already clean and used Blizzard-style capitalized aliases consistently; the one remaining lowercase fallback alias found outside ClassMods was `legacyGetSpellCooldown`, which is now normalized to `GetSpellCooldownLegacy` in `SuperSwingTimer_State.lua`. No version bump; still pinned to `v0.1.2` for final test.

- v0.1.2 alias consistency beast-mode audit (2026-05-31): audited all addon Lua files for lower/upper spell-API alias drift and normalized the remaining runtime naming to Blizzard-style `C_Spell` everywhere the local table is referenced. Confirmed the local alias itself is a readability/lookup convenience, not a meaningful performance cost; the real optimization win is avoiding repeated global lookups in hot paths and keeping one top-level alias per file. No version bump; still pinned to `v0.1.2` for final test.

- v0.1.2 warrior Overpower glow + naming consistency hardening (2026-05-31): upgraded the Warrior Overpower cue to a pulsing border glow on the MH bar instead of a flat fill tint, tied to the existing proc window sourced from dodge/parry combat events. Also normalized remaining class-mod runtime aliasing so the code uses the same `C_Spell` spelling convention instead of mixing `cSpell` and `C_Spell` variants. This was done without changing the addon version; `SuperSwingTimer.toc` remains `v0.1.2` for final test.

- v0.1.2 production shaman cast-motion + Flame Shock hardening (2026-05-31): finished the requested deep-dive work on live weave motion and Flame Shock recast warning UX. Shaman weave visuals now prefer live `UnitCastingInfo` / `UnitChannelInfo` timestamps for the current cast, then fall back to haste-adjusted spell time only when live timestamps are unavailable. Weave spark/triangle textures were moved onto a dedicated higher overlay frame above the bar stack so the red moving spark stays visible in real-time. Flame Shock helper now shows a self-resetting orange border glow during the last 4 seconds of the aura. No addon version bump; still pinned to `v0.1.2` for final user test.

- v0.1.2 production UI z-order + slider-track hotfix (2026-05-31): fixed two user-blocking config regressions in `SuperSwingTimer_Config.lua` with explicit non-demo layering. Slider tracks are now rendered through a dedicated per-row track layer frame (ARTWORK textures + deterministic frame levels) so every slider row keeps a visible track on Classic/TBC `OptionsSliderTemplate` variants. Texture browser popup now enforces `FULLSCREEN_DIALOG` + top-level behavior and reapplies high frame level/raise on show, preventing the picker from appearing behind the main config window while dragging/opening rows. TOC version remains `v0.1.2` pending final user validation.

- v0.1.2 final pre-test production audit + UI slider track pass (2026-05-31): completed a release-candidate audit focused on (1) universal slider-track visibility, and (2) full Shaman wiring correctness before user in-game validation. `SuperSwingTimer_Config.lua` slider rows now render a shared row-owned track shadow/base anchored to the live slider widget so the track is visible across Classic/TBC `OptionsSliderTemplate` variance for all sliders. Verified Shaman end-to-end wiring: event registration (`UNIT_SPELLCAST_*`, `UNIT_AURA`, `PLAYER_TARGET_CHANGED`, `SPELLS_CHANGED`) in `SuperSwingTimer.lua`, highest-known-rank weave catalog rebuild in `SuperSwingTimer_Weaving.lua`, castGUID token safety in `SuperSwingTimer_State.lua`, and runtime visuals in `SuperSwingTimer_ClassMods.lua` including red moving cast spark + cast-following weave indicators + configurable Lightning Shield tracker gap. Also corrected metadata discipline for this test cycle by restoring `SuperSwingTimer.toc` version to `v0.1.2` (no premature version bump before final test sign-off).
